...
function nextFrame() {
	...
	falling = moveByOne(1,0);
		//and if not move on to next piece
	if( !falling ) {
...
function writePiece() {
	...
	for ( i = 0; i < fallingPiecePos.length; i++ ) {
			//check if this part of new piece is valid, gameover if not
		if (board[ fallingPiecePos[i].y ][ fallingPiecePos[i].x ] != 0) {
			gameOver(); return;
		}
